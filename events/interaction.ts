        ...
        switch (Interaction.type) { 
            ...
            case Constants.InteractionTypes.APPLICATION_COMMAND: {
                ...
                if (Command) {
                    ...
                    Log("Command Executed", `<code style="background-color:rgb(36, 36, 36); padding: 2px 4px; border-radius: 4px;">${Interaction.user?.username}</code> used the <code style="background-color:rgb(36, 36, 36); padding: 2px 4px; border-radius: 4px;">/${Interaction.data.name}</code> command.`, LoggingColors.GREEN, undefined, true); // only to relay
